...
export default function Home() {
  ...
  const handleCheck = async () => {
    ...
    try {
      setLoading(true);
      const response = await axios.post("https://your-api-endpoint.com/schedule", scheduleData);
    ...
    <div className="flex w-full flex-col items-center p-4 sm:p-8 bg-gray-100">
      <div className="flex flex-col md:flex-row gap-6 w-full max-w-[1200px] bg-white p-6 sm:p-8 shadow-lg rounded-md">
        ...
        <div className="w-full md:w-[50%]">
          <h2 className="text-xl font-bold mb-4 text-center md:text-left">MAY 2025</h2>
          <Calendar selected={selectedDate} onSelect={setSelectedDate} />
        </div>

        {/* Schedule */}
        <div className="w-full md:w-[45%]">
          <h2 className="text-xl font-bold mb-4 text-center md:text-left">SCHEDULE</h2>
