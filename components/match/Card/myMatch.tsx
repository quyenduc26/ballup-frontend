      ...
      <div className="relative">
        ...
        <div className="absolute w-full px-4 sm:px-8 -bottom-12 sm:-bottom-16 md:-bottom-20 flex justify-between">
          ...
          <div className="bg-gray-100 rounded-full w-10 h-10 sm:w-14 sm:h-14 flex items-center justify-center self-end mb-2">
            <img alt="VS" className="w-8 h-8 sm:w-10 sm:h-10" src="/images/VS.png" />
          </div>
      ...
      <div className="p-2 sm:p-4 pt-14 sm:pt-20 md:pt-24">
        <div className="flex items-center justify-between mt-2">
          ...
          <div className="text-center w-1/3">
            <h2 className="sm:text-lg text-xs font-bold truncate px-1">{match.teamA?.name || "Team A"}</h2>
          </div>

          {/* Match Info */}
          <div className="text-center w-1/3">
            <p className="text-xs text-gray-500">{formatTimestamp(match.fromTime)}</p>
            <p className="text-xs sm:text-sm font-bold sm:mt-2">{formatTimestamp(match.toTime)}</p>
            <p className="sm:text-lg text-xs mt-2 text-gray-500">{match.center || "Location not set"}</p>
          </div>

          {/* Team B Name */}
          <div className="text-center w-1/3">
            <h2 className="sm:text-lg text-xs font-bold truncate px-1">{match.teamB?.name || "Team B"}</h2>
          </div>
        </div>

        {/* Action Buttons */}
        <div className="flex gap-1 sm:gap-2 mt-4 sm:mt-10 mb-2">
          <button className="border border-gray-300 w-1/3 py-2 sm:py-3 sm:text-sm text-xs  font-medium text-gray-700 rounded hover:bg-gray-200">
            EDIT
          </button>
          <button
