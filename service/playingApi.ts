...
const playingApi = {
  ...
  getAllCenter: (params?: Record<string, string>) => api.get("/center", { params }),
  getCenterInfor: (id: number) => api.get(`/center/${id}`),
  getBlockedSlot: (slotId: number, startOfDay: number, endOfDay: number) => api.get(`/unavailable-slot/${slotId}/blocked?startOfDay=${startOfDay}&endOfDay=${endOfDay}`),
